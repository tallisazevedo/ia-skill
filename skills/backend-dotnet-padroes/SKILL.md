...
|---|---|
| Entidade, invariante, regra de negócio, serviço de domínio, comando | `references/domain.md` |
| Request, Response, AppService, orquestração, async, `CancellationToken` | `references/application.md` |
| Endpoint, controller, URI, código e formato de resposta, paginação, JSON, Swagger, autenticação | `references/api.md` |
| Repositório, mapeamento NHibernate, Unit of Work, Dapper, HttpClient, filtro, consulta | `references/persistence.md` |
| Registro de dependência, ciclo de vida, bootstrapper | `references/ioc.md` |
| Teste de entidade ou serviço de domínio, cobertura | `references/testing.md` |
| Log estruturado, EventoId, nível, dado sensível | `references/observability.md` |
| Nome de classe, interface, método, Request/Response, Map, teste | `references/naming.md` |
| Onde a responsabilidade mora entre domínio, aplicação, infraestrutura e IoC | `references/architecture.md` |
...
para nomes de padrões de projeto (`Repository`, `Factory`, `Strategy`). Detalhe em
`references/naming.md`.

**Dados sensíveis não circulam.** Senha, token, segredo e credencial não entram em log, corpo
de erro ou mensagem de fila. Detalhe em `references/observability.md`.
